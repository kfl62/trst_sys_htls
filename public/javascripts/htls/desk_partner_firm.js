    ...
    $.extend(true, Htls, {
      desk: {
        partner_firm: {
            ...
            btns.each(function() {
              ...
              if ($bd.action === 'print') {
                return $button.on('click', function() {
                  ...
                  $url = "/sys/htls/report/print?rb=yearly_stats";
                  if ($bd.fn) {
                    $url += "&fn=" + $bd.fn;
                  }
                  if ($bd.uid) {
                ...
                $button.on('click', function() {
                  ...
                  if ($('input.param.doc_ary').val().split(',')[0] === "" || $('input.param.doc_ary').val().split(',').length < $('input:checkbox').length) {
                    ...
                    Htls.desk.partner_firm.doc_ary = [];
                  }
                  $('input.param.doc_ary').val(Htls.desk.partner_firm.doc_ary);
                  $params = jQuery.param($('.param').serializeArray());
                  $url = "/sys/htls/partner_firm/query?" + $params;
                  Trst.desk.init($url);
                });
          ...
          init: function() {
            this.doc_ary = [];
            Htls.desk.partner_firm.buttons($('button'));
            Htls.desk.partner_firm.selects($('select.param,input.select2'));
            Htls.desk.partner_firm.inputs($('input.doc_ary'));
            return $log('Htls.desk.partner_firm.init() OK...');
          }
        }
      }
    });
    return Htls.desk.partner_firm;
  });
