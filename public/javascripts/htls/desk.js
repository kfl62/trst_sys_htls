    ...
    $.extend(true, Htls, {
      desk: {
        ...
        idPnHandle: function() {
          ...
          if (Htls.desk.idPnValidate($input.val())) {
        ...
        init: function() {
          ...
          if ((_ref = Trst.desk.hdo.dialog) === 'create' || _ref === 'edit') {
            if ($('input[name*="id_pn"]').length) {
              Htls.desk.idPnHandle();
              $('input[name*="id_pn"]').on('keyup', function() {
                return Htls.desk.idPnHandle();
              });
            }
            $('.focus').focus();
          } else {
          ...
          if ($('table.scroll').height() > 450) {
            Htls.desk.scrollHeader($('table.scroll'));
          }
          $log('Htls.desk.init() Ok...');
          if ($('select.htls, input.select2').length) {
            require(['htls/desk_select'], function(select) {
              return select.init();
            });
          }
          if ($ext = Trst.desk.hdo.js_ext) {
            require(["htls/" + $ext], function(ext) {
              return ext.init();
            });
          }
        }
